...
for n_loops in [2]:
    ...
    for n_repeat in range(5):
        wandb.init(config=hp_defaults, project="RViT_Experiments", group='pet_full_tiny_pretrained_blurinv_m2_final')
        config = wandb.config
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        name = folder + f'/pet_tiny_pretrained_k_{n_loops}_run_{n_repeat}_method2_blurinv'
