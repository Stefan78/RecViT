...
for n_loops in [2, 3, 4]:
    ...
    for n_repeat in range(5):
        wandb.init(config=hp_defaults, project="RViT_Experiments", group='pet_full_tiny_pretrained_5x5_m1_transform_final')
        config = wandb.config
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        name = folder + f'/pet_tiny_pretrained_k_{n_loops}_run_{n_repeat}_transform'
