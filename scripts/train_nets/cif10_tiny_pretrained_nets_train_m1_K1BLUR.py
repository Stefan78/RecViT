...
for n_blurs in [1, 2, 3]:
    for n_repeat in range(5):
        wandb.init(config=hp_defaults, project="RViT_Experiments", group='CIF10_5x5_baseline_RViT_m1_k1BLUR')
        config = wandb.config
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        name = folder + f'/cif10_tiny_pretrained_k_{1}_run_{n_repeat}_random_blur_{n_blurs}'
        ...
        training.train_and_test_rvit(model, train_loader, test_loader, optimizer, device, epochs=config['n_epochs'],
                                     ...
                                     scheduler=scheduler, n_loops=1, train_option=config['train_option'],
                                     random_blur=n_blurs, blur_value=(35, 12))
        wandb.finish()
