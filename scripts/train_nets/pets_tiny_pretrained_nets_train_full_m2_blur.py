...
for n_loops in [2, 3, 4]:
    if n_loops >= 4:    # due to limited GPU
        hp_defaults['batch_size'] = 8
    for n_repeat in range(5):
        wandb.init(config=hp_defaults, project="RViT_Experiments", group='pet_full_tiny_pretrained_blur_m2_final')
        config = wandb.config
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        name = folder + f'/pet_tiny_pretrained_k_{n_loops}_run_{n_repeat}_method2_blur'
        ...
        training.train_and_test_rvit(model, train_loader, test_loader, optimizer, device, epochs=config['n_epochs'],
                                     ...
                                     random_transform=False, blur=True)
        wandb.finish()
