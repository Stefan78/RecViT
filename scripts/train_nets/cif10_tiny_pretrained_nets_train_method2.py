...
for n_loops in [1]:
    for n_repeat in range(5):
        wandb.init(config=hp_defaults, project="RViT_Experiments", group='CIF10_5x5_baseline_RViT_m2')
        config = wandb.config
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        name = folder + f'/cif10_tiny_pretrained_k_{n_loops}_run_{n_repeat}_method2'
