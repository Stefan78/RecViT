# this file is used when searching for best hyperparams for VGG
...
if __name__ == '__main__':
    pretrained_model = torchvision.models.vgg11_bn(weights=True)
    hp_defaults = dict(
        lr=0.0063,
        lr_decay=0.96,
        optimizer='SGD',
    ...
    config = wandb.config
    my_model = PETVGG(my_pretrained_model=pretrained_model)
    ...
    if save:
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        PATH = folder + f'/pet_VGG_pretrained'
