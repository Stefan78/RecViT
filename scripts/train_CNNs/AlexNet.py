# this file is used when searching for best hyperparams for AlexNet
...
if __name__ == '__main__':
    pretrained_model = torchvision.models.alexnet(weights=True)
    hp_defaults = dict(
        lr=0.0022,
        lr_decay=0.87,
        optimizer='SGD',
    ...
    config = wandb.config
    my_model = PETAlexNet(my_pretrained_model=pretrained_model)
    ...
    if save:
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        PATH = folder + f'/pet_AlexNet_pretrained'
